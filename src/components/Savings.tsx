      ...
      <div className="bg-blue-600 rounded-[3rem] p-12 md:p-16 text-white relative overflow-hidden shadow-2xl shadow-blue-600/30">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-10">
          <div className="space-y-6 text-center md:text-left">
            <h1 className="text-5xl md:text-7xl font-black tracking-tighter">Savings Goals</h1>
            <p className="text-blue-100 text-xl font-medium max-w-lg opacity-90 leading-relaxed">
              Plan for your future milestones and watch your wealth grow with Zenith's automated tracking.
            </p>
          </div>
          <button 
            onClick={() => setIsModalOpen(true)}
            className="bg-white text-blue-600 px-10 py-6 rounded-[2rem] font-black text-lg flex items-center gap-3 hover:bg-blue-50 hover:scale-105 active:scale-95 transition-all shadow-2xl shadow-black/10 group"
          >
        ...
        {goals.map((goal) => {
          ...
          return (
            <motion.div 
              layout
              key={goal.id} 
              className="bg-white p-10 rounded-[3rem] border-2 border-gray-50 shadow-sm space-y-10 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group"
            >
              ...
              <div>
                <h3 className="text-3xl font-black text-gray-900 mb-2 tracking-tight">{goal.name}</h3>
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-end">
                  <div className="space-y-1.5">
                    ...
                    <p className="text-3xl font-black text-gray-900 tracking-tighter">${(goal.currentAmount ?? 0).toLocaleString()}</p>
                  </div>
                  <div className="text-right space-y-1.5">
                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Target</p>
                    <p className="text-xl font-black text-gray-300 tracking-tighter">${(goal.targetAmount ?? 0).toLocaleString()}</p>
                  </div>
                </div>
                
                <div className="space-y-3">
                  <div className="h-5 bg-gray-50 rounded-full overflow-hidden p-1">
                    <motion.div 
                      initial={{ width: 0 }}
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <p className="text-sm font-black text-blue-600">{Math.round(progress)}% Complete</p>
                  </div>
                </div>
              </div>

              <div className="flex items-center gap-3 text-xs text-gray-500 font-bold bg-gray-50 p-4 rounded-2xl border border-gray-100">
                <Calendar className="w-5 h-5 text-blue-600" />
                <span>Target: {new Date(goal.deadline).toLocaleDateString(undefined, { month: 'long', day: 'numeric', year: 'numeric' })}</span>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <button 
                  onClick={() => handleAddFunds(goal, 50)}
                  className="py-5 bg-gray-50 text-gray-900 rounded-[1.5rem] font-black text-sm hover:bg-gray-100 transition-all active:scale-95"
                >
                  Add $50
                </button>
                <button 
                  onClick={() => handleAddFunds(goal, 100)}
                  className="py-5 bg-blue-600 text-white rounded-[1.5rem] font-black text-sm hover:bg-blue-700 transition-all shadow-xl shadow-blue-600/20 active:scale-95"
                >
                  Add $100
                </button>
              </div>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-xl z-[100] flex items-center justify-center p-6">
            <motion.div 
              ...
              className="bg-white w-full max-w-lg rounded-[3.5rem] shadow-2xl p-12 relative overflow-hidden"
            >
              <div className="absolute top-0 right-0 w-48 h-48 bg-blue-50 rounded-full -mr-24 -mt-24 blur-3xl opacity-50"></div>
              
              <button 
                onClick={() => setIsModalOpen(false)}
                className="absolute top-10 right-10 text-gray-300 hover:text-gray-900 transition-colors p-2 hover:bg-gray-50 rounded-xl"
              >
                <X className="w-7 h-7" />
              </button>

              <h2 className="text-4xl font-black text-gray-900 mb-10 tracking-tighter">New Goal</h2>
              ...
              <form onSubmit={handleCreateGoal} className="space-y-8">
                <div className="space-y-3">
                  ...
                  <input 
                    ...
                    className="w-full px-8 py-5 rounded-[2rem] bg-gray-50 border-2 border-transparent focus:bg-white focus:border-blue-600 outline-none font-bold text-gray-900 transition-all"
                    value={newGoal.name}
                ...
                <div className="grid grid-cols-2 gap-8">
                  ...
                  <div className="space-y-3">
                    ...
                    <select 
                      className="w-full px-8 py-5 rounded-[2rem] bg-gray-50 border-2 border-transparent focus:bg-white focus:border-blue-600 outline-none font-bold text-gray-900 transition-all appearance-none"
                ...
                <button 
                  ...
                  className="w-full bg-blue-600 text-white py-6 rounded-[2rem] font-black text-xl mt-6 hover:bg-blue-700 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-3 shadow-2xl shadow-blue-600/30 disabled:opacity-50"
                >
